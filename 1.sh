...
# === OpenWrt Checker v3.4 (UTF-8 Safe) ===
# Исправлено: идеальная кириллица без ???? через байтовую фильтрацию awk
...
[[ -z "$RAW" ]] && { echo "Ошибка скачивания"; exit 1; }
echo "OK"
...
# Шапка таблицы (строгое выравнивание)
echo "----------------------------------------------------------------------------------------------------"
...
for link in $LINKS; do
    ...
    # --- ТЕХНОЛОГИЯ ОЧИСТКИ БЕЗ ???? ---
    # Мы используем awk для фильтрации байтов. 
    # Оставляем: ASCII (0-127) и 2-байтовую кириллицу (C0-DF + 80-BF). 
    # Всё остальное (эмодзи) удаляется без вреда для букв.
    CLEAN_NAME=$(echo "$NAME" | awk '{gsub(/[^\x00-\x7F\xC0-\xDF\x80-\xBF]/,"")}1' | sed 's/[|@]/-/g')
    ...
    # Печать строки (используем %b для цветов)
